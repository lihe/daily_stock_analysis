...
const settingsHelpZhCN: SettingsHelpMap = {
  ...
  'settings.data_source.TUSHARE_API_URL': {
    ...
    summary: '配置内置 Tushare HTTP client 请求的 Pro 兼容接口地址。',
    usage: '官方服务保持默认值；使用兼容服务时填写服务方提供的完整 HTTP 或 HTTPS 地址。',
    valueNotes: ['该配置只影响分析运行时的内置 Tushare HTTP client。'],
    impact: ['影响 Tushare 日线、筹码和行业资金流等兼容接口请求的目标地址。'],
    notes: ['Token 仍应单独保存在 TUSHARE_TOKEN，不要拼接到 URL。'],
  },
  'settings.data_source.TUSHARE_BYPASS_PROXY': {
    title: 'Tushare 绕过代理',
    summary: '控制内置 Tushare HTTP client 是否忽略系统代理配置。',
    usage: '仅当兼容服务必须直连时开启；默认关闭以保持既有网络行为。',
    valueNotes: ['开启后只修改 Tushare 专用 Session 的 trust_env，不写入全局 NO_PROXY。'],
    impact: ['不会改变模型、搜索、通知和其他行情数据源的代理行为。'],
    notes: ['若运行环境必须经代理访问该接口，请保持关闭。'],
  },
...
const settingsHelpEnUS: SettingsHelpMap = {
  ...
  'settings.data_source.TUSHARE_API_URL': {
    ...
    summary: 'Sets the Pro-compatible endpoint used by the built-in Tushare HTTP client.',
    usage: 'Keep the default for the official service, or enter the full HTTP/HTTPS URL supplied by a compatible provider.',
    valueNotes: ['This setting affects only the built-in Tushare HTTP client used during analysis.'],
    impact: ['Changes the endpoint for compatible daily, chip, and industry money-flow requests.'],
    notes: ['Keep the token in TUSHARE_TOKEN; never append it to the URL.'],
  },
  'settings.data_source.TUSHARE_BYPASS_PROXY': {
    title: 'Tushare Proxy Bypass',
    summary: 'Controls whether the built-in Tushare HTTP client ignores system proxy settings.',
    usage: 'Enable only when the compatible endpoint must be reached directly. The default preserves existing network behavior.',
    valueNotes: ['When enabled, only the Tushare session disables trust_env; global NO_PROXY is not changed.'],
    impact: ['Does not change proxy behavior for model, search, notification, or other market-data requests.'],
    notes: ['Leave disabled when the endpoint must be reached through the configured proxy.'],
  },
  'settings.data_source.TICKFLOW_API_KEY': {
    title: 'TickFlow API Key',
